...
public class LTCrashHandler implements Thread.UncaughtExceptionHandler {
    ...
    private static final String TAG = LTCrashHandler.class.getSimpleName();
   ...
   /*    if (ex == null) {
        ...
        new Thread() {
            ...
            public void run() {
                Looper.prepare();
                Toast.makeText(mContext, "很抱歉,程序数据出现异常,即将退出.请查看《张家港移动办公》/异常信息日志 目录下的日志信息。", Toast.LENGTH_LONG).show();
                Looper.loop();
            }
        }.start();
        //收集设备参数信息
    ...
    private File getCrashLogFolder() {
        if (_crashLogDirFile == null) {
            ...
            _crashLogDirFile = new File(SDCardRoot, "万户/异常信息日志/");
